...
.. class:: _parDict
   ...
   The lookup function attempts to be clever and convert the parameters to floats if possible; otherwise, the parameters are stored as strings. There are currently two exceptions to this rule, which are ``TD`` and ``SI``. These are stored as ints.

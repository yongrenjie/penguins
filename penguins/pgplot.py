...
def cleanup_yaxes(ax):
    # Need to draw the figure to get the renderer, whatever that means.
    fig = ax.figure
    fig.canvas.draw()
    r = fig.canvas.get_renderer()
    # Iterate over axes and check which yticks overlap with the label.
    for ax in axs.flat:
        ylabel_bbox = ax.yaxis.label.get_window_extent(renderer=r)
        for ytick in ax.yaxis.get_major_ticks():
            ytick_bbox = ytick.label2.get_window_extent(renderer=r)
            if ytick_bbox.overlaps(ylabel_bbox):
